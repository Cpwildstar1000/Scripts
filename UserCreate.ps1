...
New-ADUser -Name $Name -AccountPassword (ConvertTo-SecureString "$Password" -AsPlainText -Force) -Company $Company -DisplayName $Name -Enabled $true -GivenName $FirstName -Path $Path -SamAccountName $UserName -ScriptPath $Script -Surname $LastName -UserPrincipalName $UserName
...
$Acl = Get-Acl $Comp
$Ar = New-Object System.Security.AccessControl.FileSystemAccessRule ("cpulvermacher", "FullControl", "Allow")
...
New-ADUser -Name $Name -AccountPassword (ConvertTo-SecureString "$Password" -AsPlainText -force) -Company $Company -DisplayName $Name -GivenName $FirstName -Path $Path -SamAccountName $UserName -ScriptPath $Script -Surname $LastName -Enabled $true
